...
public class FileServiceImpl implements FileService {
    ...
    public Files selectFileById(int no) throws Exception {
        return fileMapper.selectFileById(no);
    }
    ...
    public int deleteFile(int no) throws Exception {
        return fileMapper.deleteFile(no);
    }
    ...
    public Files uploadFile(Files fileInfo, MultipartFile file) throws Exception {
        ...
        result = fileMapper.insertFile(uploadedFile);
    ...
    public int download(int no, HttpServletResponse response) throws Exception {
        Files file = fileMapper.selectFileById(no);

        if ( file == null) {
            // BAD_REQUEST : 400, 클라이언트의 요청이 잘못 되었음을 알리는 상태코드
